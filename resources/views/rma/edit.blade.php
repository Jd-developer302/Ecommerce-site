...
@section('content')
    ...
    <div class="page-header">
        ...
        <div>
            <ol class="breadcrumb">
                <a href="{{ route('rma.index') }}" class="btn btn-sm btn-primary my-1">Back</a>
            </ol>
        </div>
    </div>
    <!-- PAGE-HEADER END -->
    <div class="card mt-3 p-4">
                    ...
                    <tbody>
                        @foreach ($previous as $item)
                            <tr>
                                ...
                                <td class="tdClass">
                                    <button class="btn btn-sm ">{{ $item->status }}</button>
                                </td>
        ...
        @endif
        <div class="card-body">
            <form method="POST" action="{{ route('rma.update', $rma->id) }}" enctype="multipart/form-data" class="test-form">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col-1">
                            <select name="areacode" id="areacode" value="{{ $rma->areacode }}" class="form-control"
                                required>
                                <option value="">Area Code</option>
                                <option value="050" {{ old('areacode', $rma->areacode) == '050' ? 'selected' : '' }}>
                                    050</option>
                                <option value="052" {{ old('areacode', $rma->areacode) == '052' ? 'selected' : '' }}>
                                    052</option>
                                <option value="054" {{ old('areacode', $rma->areacode) == '054' ? 'selected' : '' }}>
                                    054</option>
                                <option value="055" {{ old('areacode', $rma->areacode) == '055' ? 'selected' : '' }}>
                                    055</option>
                                <option value="056" {{ old('areacode', $rma->areacode) == '056' ? 'selected' : '' }}>
                                    056</option>
                                <option value="058" {{ old('areacode', $rma->areacode) == '058' ? 'selected' : '' }}>
                                    058</option>
                            </select>
                        </div>
                        <div class="col-11">
                            <input value="{{ $rma->phone }}" type="number" id="phone" class="form-control"
                                name="phone" placeholder="Phone" minlength="7" maxlength="7" required autofocus
                                oninput="enforceMaxLength(this)">
                        </div>
                    </div>
                    <input type="hidden" name="order_id" value="{{ $rma->order_id }}">

                    @if ($errors->has('phone'))
                        <span class="text-danger text-left">{{ $errors->first('phone') }}</span>
                    @endif
                </div>
                <input type=" hidden" name="status" class="form-control mt-4" value="RMA">
                <div class="mb-3">
                    <label for="name" class="form-label">Customer Name</label>
                    <input type="text" id="name" class="form-control" name="name" value="{{ $rma->name }}"
                ...
                <div class="mb-3">
                    ...
                    <input type="email" id="email" class="form-control" name="email" value="{{ $rma->email }}"
                        placeholder="Email">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="city" id="city" required>
                        ...
                        <option value="DUBAI" @if ('DUBAI' == $rma->city) selected @endif>DUBAI</option>
                        <option value="ABU DHABI" @if ('ABU DHABI' == $rma->city) selected @endif>ABU DHABI
                        </option>
                        <option value="SHARJAH" @if ('SHARJAH' == $rma->city) selected @endif>SHARJAH</option>
                        <option value="AJMAN" @if ('AJMAN' == $rma->city) selected @endif>AJMAN</option>
                        <option value="RAK" @if ('RAK' == $rma->city) selected @endif>RAK</option>
                        <option value="FUJAIRAH" @if ('FUJAIRAH' == $rma->city) selected @endif>FUJAIRAH</option>
                        <option value="UMM AL QUWAIN" @if ('UMM AL QUWAIN' == $rma->city) selected @endif>UMM AL QUWAIN
                        </option>
                        <option value="AL AIN" @if ('AL AIN' == $rma->city) selected @endif>AL AIN</option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="source" id="source" required>
                        <option value="">-Select-</option>
                        <option value="Website" @if ('Website' == $rma->source) selected @endif>Website</option>
                        <option value="Message" @if ('Message' == $rma->source) selected @endif>Message</option>
                        <option value="Leads" @if ('Leads' == $rma->source) selected @endif>Leads</option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea type="text" id="address" class="form-control" name="address" value="{{ $rma->address }}"
                        placeholder="Address" required>{{ $rma->address }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col-6"><input value="{{ $rma->lat }}" type="number" id="lat"
                                class="form-control" name="lat" placeholder="Latitude"></div>
                        <div class="col-6"><input value="{{ $rma->long }}" type="number" id="long"
                        ...
                        <table class="table table-bordered" id="dynamicAddRemove">
                            ...
                            @foreach ($rma_linesp as $item)
                                <tr>
                                    ...
                                    <td class="tdClass"><a href="{{ route('delete.cartitems', [$item->id, $rma->id]) }}"
                                        type="button"
                                        class="btn btn-outline-danger remove-input-field">Delete</button></a></td>
                                </tr>
                            @endforeach
                            @foreach ($rma_linesb as $item)
                                <tr>
                                    <td class="tdClass">{{ $item->sku }}</td>
                                    <td class="tdClass"><img src="{{ URL::asset('image/' . $item->image) }}"
                ...
                <div class="mb-3">
                    ...
                    <textarea value="{{ $rma->comment }}" type="text" class="form-control" name="comment" placeholder="Comment">{{ $rma->comment }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <div class="row">
                        <input type="text" value="{{ $rma->coupon_code }}" name="coupon_code"
                        ...
                        <input type="hidden" name="is_coupon_applied" id="is_coupon_applied"
                            value="{{ $isApplied }}">
                        <input type="hidden"name="coupon_price" id="coupon_price" value="{{ $rma->coupon_price }}">
                        <input type="hidden" name="coupon_id" id="coupon_id" value="{{ $rma->coupon_id }}">
                    </div>
                ...
                <a href="{{ route('rma.index') }}" class="btn btn-default">Back</a>
            </form>
        </div>
        ...
        <script type="text/javascript">
            ...
            $("#dynamic-ar").click(function() {
                ...
                $("#dynamicAddRemove").append(
                    '<tr><td class="tdClass"  align="center"><input type="text" name="RmaLine[' + j +
                    '][sku]" id="sku" class="form-control sku">' +
                    '<td class="tdClass"  align="center" width="20%"><select name="RmaLine[' +
                    j +
                    '][product_id]" id="productid" class="mySelect js-example-basic-single form-control" required>' +
                    options +
                    ' </select></td><td class="tdClass" align="center"><input type="text" name="RmaLine[' + j +
                    '][price]" placeholder="Enter Price" value="" class="form-control unit-price" readonly/><input type="hidden" name="RmaLine[' +
                    j +
                    '][name]" placeholder="Enter Price" value="" class="form-control product-name"/><input type="hidden"  value="" class="form-control delivery-charge"/></td><td class="tdClass" align="center"><input type="number" name="RmaLine[' +
                    j +
                    '][quantity]" placeholder="Quantity" value="0" class="form-control quantity" min="1" required /></td><td class="tdClass" align="center"><input type="text" name="RmaLine[' +
                    j +
            ...
            $("#dynamic-ar1").click(function() {
                ...
                $("#dynamicAddRemove").append(
                    '<tr><td class="tdClass"  align="center"><input type="text" name="RmaLine[' + j +
                    '][sku]" id="sku" class="form-control sku">' +
                    '<td class="tdClass"  align="center" width="20%"><select name="RmaLine[' + j +
            ...
            function updateTotalSum() {
                var totalSum = <?= $rma->total_price ?>;
                var dcold = <?= $rma->delivery_charge ?>;
                $(".total-price").each(function() {
                    totalSum += parseFloat($(this).val());
                });
                $(".sub-total").val(totalSum);
                var lowestDeliveryCharge = <?= $rma->delivery_charge ?>;
